...
export class OrderPage {
  ...
  getToken = function () {
    this.loading.autoHide(2000);
    this.httpClient.get(this.config.url + 'generatebraintreetoken').subscribe((data: any) => {
      // this.loading.hide();
      if (data.success == 1) {
  ...
  instamojoPayment() {
    ...
    // this.instamojoClient = new Instamojo(this.http, this.iab, this.config.url + 'instamojotoken');
